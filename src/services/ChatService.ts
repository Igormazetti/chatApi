...
export class ChatService {
  ...
  async sendMessage(params: SendMessageParams): Promise<ServiceResponse<Message>> {
    try {
      ...
      if (receiverId) {
        ...
        const receiverIsMember = await this.roomModel.isUserMember(roomId, receiverId);
        if (!receiverIsMember) {
          return { status: 403, error: 'Receiver is not a member of this room' };
        }
      }

      if (replyTo) {
        const originalMessage = await this.messageModel.findMessageById(replyTo);
        if (!originalMessage) {
          return { status: 404, error: 'Original message not found' };
        }
        if (originalMessage.room_id !== roomId) {
          return { status: 400, error: 'Cannot reply to message from different room' };
        }
      }
  ...
  async getRoomMessages(params: GetRoomMessagesParams): Promise<ServiceResponse<Message[]>> {
    try {
  ...
  async editMessage(params: EditMessageParams): Promise<ServiceResponse<Message>> {
    try {
  ...
  async deleteMessage(params: DeleteMessageParams): Promise<ServiceResponse<null>> {
    try {
      ...
      if (message.sender_id !== userId) {
        return { status: 403, error: 'User not allowed to delete this message' };
      }
  ...
  async replyToMessage(params: ReplyToMessageParams): Promise<ServiceResponse<Message>> {
    try {
      ...
      const isMember = await this.roomModel.isUserMember(originalMessage.room_id, senderId);
      if (!isMember) {
        return { status: 403, error: 'User is not a member of this room' };
      }
